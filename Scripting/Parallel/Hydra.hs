...
module Scripting.Parallel.Hydra
       (
...
-- | Run a set of commands in parallel, but bounded to a certain number of worker
-- threads.  Returns
--   (1) a list of streams that are ready to be fed to HydraPrint
--   (2) a wait action that returns the exit codes of all processes.
runBatchCmds :: Int -> [String] -> IO ([S.InputStream B.ByteString], IO [ExitCode])
runBatchCmds numWorkers cmds =
  error "implement runBatchCmds"
